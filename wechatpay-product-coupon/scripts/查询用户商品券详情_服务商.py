...
"""
服务商 - 查询用户商品券详情
API文档：https://pay.weixin.qq.com/doc/partner/4013080339
...
def build_authorization(mchid: str, serial_no: str, method: str, uri: str,
                        signature: str, timestamp: str, nonce_str: str, body: str = "") -> str:
    """
    构造完整的 Authorization 头（服务商签名方案）。
...
def print_coupon_analysis(data: dict):
    ...
    if state == "EXPIRED":
        ...
        print("   → 注意：用户卡包删券没有回调通知，商户/服务商侧无法实时感知。")
...
def main():
    parser = argparse.ArgumentParser(
        description="服务商 - 查询用户商品券详情",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python3 查询用户商品券详情_服务商.py \\
    --mchid 1900000001 \\
    --serial-no 5A6B7C8D9E0F1A2B3C4D5E6F7A8B9C0D1E2F3A4B \\
    ...
    # 服务商配置参数
    parser.add_argument("--mchid", required=True, help="服务商商户号")
    parser.add_argument("--serial-no", required=True, help="服务商API证书序列号")
    ...
    # ====== 构造请求 URL ======
    # 服务商 API 路径前缀为 /v3/，且查询参数额外需要 brand_id
    uri = (
        f"/v3/marketing/partner/product-coupon/users/{quote(args.openid, safe='')}"
        ...
        f"&brand_id={quote(args.brand_id, safe='')}"
    )
