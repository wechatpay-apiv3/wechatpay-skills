...
| 33 | **自费 JSAPI 下单 `amount.total` 字段值** | 服务商自费 JSAPI 下单调用前的 `amount` 构造 | 把混合单的 `total_fee`（含医保个账 `med_ins_self_fee`）传给 `amount.total`，而非 `wechat_pay_cash_fee`，导致自费 `prepay_id` 锁定金额与混合单期望自费金额不一致触发金额校验失败 | 🔴 致命 |
| 34 | **city_id 与 pay_auth_no 城市编码一致** | 创单参数构造 + 授权链路代码 | `city_id` 用了 A 城市编码（如 `140100` 太原），但 `pay_auth_no` 来自 B 城市（如 `140200` 大同），触发"未找到对应授权信息，无法查询订单信息" | 🔴 致命 |
| 35 | **订单最终态以第 2 次混合单回调为准** | 自费回调 + 混合单回调的业务处理代码（按 `sub_mchid` 路由后） | 收到第 1 次自费回调（自费支付成功）就把订单标记完成，未等第 2 次混合单回调 `mix_pay_status = MIX_PAY_SUCCESS`；医保结算可能失败 / 冲正后需退自费款，过早标完成会漏退 | 🔴 致命 |
| 36 | **前端低版本 / 平台兼容判断** | 调起前端代码（子商户小程序 `wx.requestMedicalInsurancePay` / 公众号 H5 JSSDK） | 未用 `wx.checkJsApi`（公众号）或 `typeof wx.requestMedicalInsurancePay === 'function'`（小程序）做兼容判断；低版本微信 / PC 微信 / 企业微信 / iPad 微信均不支持，直接调用会无报错无回调 | 🟡 推荐 |
| 37 | **纯医保单（INSURANCE_ONLY）跳过自费 JSAPI 下单** | 业务流程 / `mix_pay_type` 路由代码 | `INSURANCE_ONLY` 场景仍调用服务商自费 JSAPI 下单生成 `prepay_id`，浪费 `prepay_id` 且后续混合单字段联动校验必失败（`INSURANCE_ONLY` 禁传 `prepay_id`） | 🟡 推荐 |
| 38 | **授权与下单环境一致** | 授权链路 + 下单接口调用 | 授权阶段走测试环境（如 `city_id=440702` mock 接口）拿到的 `pay_auth_no`，下单时 `med_ins_test_env=false` 走正式环境；正式环境医保局找不到测试环境的授权信息 | 🔴 致命 |
