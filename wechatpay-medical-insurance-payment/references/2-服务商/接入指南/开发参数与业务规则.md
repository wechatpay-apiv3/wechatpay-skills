...
> ‼️ 间连模式下，权限位 `494` / `1295` 是针对**子商户的 `sub_appid`** 申请开通，不是服务商主体 `sp_appid`。
...
| ---- | -------- | ---- |
| iOS | 微信 ≥ 8.0.44 | 支持 |
| Android | 微信 ≥ 8.0.44 | 支持 |
| 鸿蒙（HarmonyOS） | 微信 ≥ 8.0.13 | 支持 |
| PC 版微信（Windows / Mac） | — | ❌ 不支持，报 `requestMedicalInsurancePay:fail:not supported` |
| 企业微信 / iPad 微信 | — | ❌ 不支持 |
...
- `wx.config` 的 `jsApiList` 必须包含 `requestMedicalInsurancePay`
- 在 `wx.ready` 回调中调用收银台
- 签名 URL 必须是真正发起 HTTP 请求的 URL（iOS SPA 场景需缓存首次进入页面 URL）
- 需开通 JSAPI 权限位 `494`

**常见报错**：

- `system:access_denied`：JSAPI 鉴权错误，检查 `wx.config` 和 `jsApiList`
- `config:invalid signature`：签名 URL 不正确（尤其 iOS 端）
- `system:function_not_exist`：微信客户端版本过低

**iOS SPA 注意事项**：iOS 微信内核使用首次进入页面 URL 作为签名校验基准，应在应用入口缓存 URL。约 30% 订单失败可能与 iOS 用户占比相关。

### 8.3 小程序接入

- 调用 `wx.requestMedicalInsurancePay()` 拉起收银台
- 需开通小程序权限位 `1295`（`WXA_JSAPI_INDEX_requestMedicalInsurancePay`）
- 低版本兼容：检查 `wx.requestMedicalInsurancePay` 是否存在

**小程序报错**：

- `errno:102, "access denied"`：未开通 JSAPI 权限 `1295`
- `requestMedicalInsurancePay:fail:not supported`：非手机微信环境（如 PC 端小程序）

### 8.4 LiteApp 前端 fail 回调错误码

商户在 `wx.requestMedicalInsurancePay` 的 fail 回调中看到的是 `msg` 字段中的中文文本，`errMsg` 固定为 `requestMedicalInsurancePay:fail`，具体原因看 `msg` 字段：

| msg 字段文本 | 含义 | 处理建议 |
| ------------ | ---- | -------- |
| 缺少 `mix_trade_no` | 未传 `mixTradeNo` 参数 | 检查拉起参数 |
| 缺少自费参数 | 自费参数缺失 | 检查 `prepay_id` 等自费参数 |
| 混合支付超时 | 支付流程超时 | 重新下单 |
| 订单状态为支付失败 | 自费支付已失败 | 检查自费单状态 |
| 订单状态异常 | 订单状态不明确 | 查单确认状态 |
| 自费支付失败 | 自费环节失败 | 检查支付参数和余额 |
| 用户直接退出 | 用户主动关闭 | 用户操作，无需处理 |
| 订单已关单 | 订单已超时关闭 | 重新下单 |