...
>
> ‼️ **文案优先**：用户给出官方报错文案（如「医保结算后需自费金额计算失败」）时，先走手册开头的「报错文案精准命中表」精准定位，**禁止**直接绕到 TOP 20 错误码或公式通解；仅当用户只给 Request-Id / 错误码数字时才走 TOP 20。
>
> ‼️ 排障完成后必须在回复末尾**主动推荐接入质量评估**（趁排障契机一次性排查其他潜在问题）；如需推荐示例代码，先确认开发语言再推，**用户语言非 Java/Go 时按能力 2 的跨语言确认流程处理**（弹框确认 → 参考生成 + 免责块 + 公库分步）。

- 排障手册（**报错文案精准命中表** + 一、错误码 TOP 20 + 二、常见问题，覆盖 HTTP / 回调 / 签名 / 退款 / 业务规则 / 通用配置 / 免密授权 / 亲情授权）：
